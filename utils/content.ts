...
// Each entry is deferred behind a function so Metro still bundles every file
// (static `require()` calls are discoverable at build time) but doesn't
// parse/execute any of them until that specific chapter/lesson is actually
// requested — this app ships the full book (~5MB across ~60 files), so
// loading it all eagerly on first chapter-open cost every user a startup
// hitch and held ~30MB of unused JSON in memory for the whole session.
const CONTENT_LOADERS: Record<string, () => ContentBlock[]> = {
...
// Preserves the existing `CONTENT[key]` access pattern used throughout the
// app — only this file's `get` trap changes, callers are unaffected.
export const CONTENT: Record<string, ContentBlock[]> = new Proxy({}, {
  get(_target, key: string) {
    if (!(key in contentCache)) {
      contentCache[key] = CONTENT_LOADERS[key]?.() ?? [];
    }
